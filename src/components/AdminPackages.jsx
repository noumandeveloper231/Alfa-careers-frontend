...
const AdminEmployeePackages = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingPackage) {
                const { data } = await axios.patch(`${backendUrl}/api/admin/packages/employee/${editingPackage._id}`, payload);
                if (data.success) {
                ...
                const { data } = await axios.post(`${backendUrl}/api/admin/packages/employee`, payload);
                if (data.success) {
                    toast.success(data.message);
                    await getPackages();
                    setIsModalOpen(false);
    ...
    const deletePackage = async (id, name) => {
        ...
        try {
            const { data } = await axios.delete(`${backendUrl}/api/admin/packages/employee/${id}`);
            if (data.success) {
                toast.success(data.message);
                await getPackages();
            }
    ...
    const toggleStatus = async (id) => {
        try {
            const { data } = await axios.patch(`${backendUrl}/api/admin/packages/employee/${id}/toggle-status`);
            if (data.success) {
                toast.success(data.message);
                await getPackages();
            }
            ...
            <DataTable
                columns={[
                    ...
                    { header: 'Price', render: (pkg) => `${pkg.currency} ${pkg.price.toLocaleString()}` },
                    {
                        ...
                        render: (pkg) => (
                            <span className="px-3 py-1 rounded-full text-xs font-semibold bg-blue-100 text-blue-700">{pkg.jobPostings}</span>
                        ),
                    },
                    {
                        header: 'Features',
                ...
                loading={false}
                emptyMessage="No employee packages found."
                    ...
                    <div className="bg-white rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                        <div className="sticky top-0 bg-white border-b border-gray-200 p-6 flex items-center justify-between">
                            <h2 className="text-xl font-bold text-gray-800">
                                {editingPackage ? "Edit Employee Package" : "Create New Employee Package"}
                            </h2>
...
export default AdminEmployeePackages;
